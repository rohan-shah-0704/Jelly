...
class NumJellyEstimator:
    ...
    def set_world_pop(self, people):
        ...
        assert type(people) is int, \
            "Error: Population must be set as an integer."
        # NE24: Add a test for value here
        if people < 0.0:
            print ("Error: World Population can not be a negative number")
            sys.exit()
        # Store the fraction.
    ...
    def set_frac_ppl_loving_pink(self, frac):
        ...
        # NE24: Add a test for type here
        assert type(frac) is float, \
            "Error: Fraction of People that Love Pink must be set as a float."
        # NE24: Add a test for value here
        if ((frac > 1.0) or (frac < 0.0)):
            print ("Error: Fractions can not be above the value of 1 or below the value of 0.")
            sys.exit()
        # Store the fraction.
